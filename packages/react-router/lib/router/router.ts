...
export function createRouter(init: RouterInit): Router {
  ...
  async function handleAction(
    ...
    let dataStrategyMatches = getTargetedDataStrategyMatches(
      request,
      matches,
      actionMatch,
      scopedContext
  ...
  function getLoaderDataStrategyMatches(
    request: Request,
    currentMatches: AgnosticDataRouteMatch[],
    matches: AgnosticDataRouteMatch[],
    scopedContext: unstable_RouterContextProvider,
    ...
    return matches.map((match, index) => {
      ...
      let { route } = match;
      if (!boundaryMatches.some((m) => m.route.id === route.id)) {
        dontLoad = true;
      } else if (route.lazy) {
        ...
        let defaultShouldRevalidate = shouldSkipRevalidation
          ...
          : // Forced revalidation due to submission, useRevalidator, or X-Remix-Revalidate
            ...
            isNewRouteInstance(currentMatches[index], match);
      ...
      let dsMatch: DataStrategyMatch = {
        ...
        shouldCallHandler(defaultShouldRevalidate) {
          if (!isRevalidation || !shouldRevalidateArgs) {
            return shouldLoad;
          }
          return shouldRevalidateLoader(match, {
            ...shouldRevalidateArgs,
            defaultShouldRevalidate: defaultShouldRevalidate ?? shouldLoad,
          });
        },
        resolve(handlerOverride) {
          ...
          if (dontLoad) {
            // FIXME: this shouldn't need to be async
  ...
  function getRevalidatingFetchers(
    matches: AgnosticDataRouteMatch[],
    ...
    fetchLoadMatches.forEach((f, key) => {
      ...
      if (fetchRedirectIds.has(key)) {
        ...
        shouldLoad = isRevalidationRequired;
      } else {
        isRevalidation = true;
        // Otherwise fall back on any user-defined shouldRevalidate, defaulting
        // to explicit revalidations only
        shouldLoad = shouldRevalidateLoader(fetcherMatch, {
          ...shouldRevalidateArgs,
      ...
      let dsMatches = fetcherMatches.map((match, index) => {
        if (match.route.id !== fetcherMatch.route.id) {
          let dsMatch: DataStrategyMatch = {
            ...match,
            shouldLoad: false,
        ...
        let dsMatch: DataStrategyMatch = {
          ...
          shouldLoad,
          shouldCallHandler(defaultShouldRevalidate) {
            if (!isRevalidation || shouldRevalidateArgs == null) {
              return shouldLoad;
            }
            return shouldRevalidateLoader(match, {
              ...shouldRevalidateArgs,
      ...
      if (shouldLoad || isRevalidation) {
        revalidatingFetchers.push({
          key,
          routeId: f.routeId,
          path: f.path,
          matches: dsMatches,
          match: dsMatches.find((m) => m.route.id === fetcherMatch.route.id)!,
  ...
  async function handleLoaders(
    ...
    let shouldRevalidateArgs = !initialHydration
    ...
    let dataStrategyMatches = getLoaderDataStrategyMatches(
      request,
      state.matches,
      matches,
      scopedContext,
    ...
    let revalidatingFetchers = shouldRevalidateArgs
      ? getRevalidatingFetchers(
          matches,
          routesToUse,
          scopedContext,
          shouldRevalidateArgs,
    ...
    if (
      !init.dataStrategy &&
      dataStrategyMatches.filter((m) => m.shouldLoad).length === 0 &&
  ...
  async function fetch(
    ...
    let shouldRevalidateArgs = getShouldRevalidateArgs(
      init.history,
      state.location,
      state.matches,
      normalizedPath,
  ...
  async function handleFetcherAction(
    ...
    let revalidatingFetchers = getRevalidatingFetchers(
      matches,
      routesToUse,
      scopedContext,
      shouldRevalidateArgs,
  ...
  async function callLoadersAndMaybeResolveData(
    ...
    let fetcherResultsPromise = Promise.all(
      fetchersToLoad.map(async (f) => {
        if (f.matches && f.match && f.controller && f.request) {
          let results = await callDataStrategy(
            f.request,
            f.matches,
            scopedContext,
...
export function createStaticHandler(
  ...
  function getLoaderDataStrategyMatches(
    ...
    return boundaryMatches.map((match, index) => {
      let shouldLoad = match.route.lazy != null || match.route.loader != null;
      let dsMatch: DataStrategyMatch = {
        ...match,
        shouldLoad,
  ...
  // FIXME: Duped from createRouter - clean this up
  function getTargetedDataStrategyMatches(
    request: Request,
    matches: AgnosticDataRouteMatch[],
    targetMatch: AgnosticDataRouteMatch,
